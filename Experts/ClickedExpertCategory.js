...
const ClickedExpertCategory =({route ,navigation }) => {
...
const getItems = () => {
  if (endReached) {
    ...
    setIsLoading(true);
    const url = EndPoint + `/GetAllExpertCategory/?id=${id}&page=${current_page}&page_size=2`;
   //const url = `https://dimosojunior.pythonanywhere.com/GetAllArticlesCategory/?id=${id}&page=${current_page}&page_size=2`
   ...
   <View style={[globalStyles.wrapImageContainer,{backgroundColor:theme.backgroundColor}]}>
      ...
      { item.StudentImage ? (
        <Image
          ...
          source={{uri: EndPoint + '/' + item.StudentImage}}
      ...
      <TouchableOpacity 
      // onPress={() => 
      //   navigation.navigate('Read Expert', item)
      // }
...
<TouchableOpacity>
<Text style={[globalStyles.universityheadertext,{color:theme.color}]}>{CategoryName}</Text>
</TouchableOpacity>
  ...
  <Text style={globalStyles.noitemText}>No any {CategoryName} !
  </Text>
...
export default ClickedExpertCategory;
