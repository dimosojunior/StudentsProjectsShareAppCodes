...
const PeopleWorksCategory =({navigation }) => {
...
const getItems = () => {
  if (endReached) {
   ...
   const url = EndPoint + `/GetAllPeopleWorksCategory/?page=${current_page}&page_size=2`
   // console.log(url);
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
 ...
 // mwanzo kwa ajili ya search
    ...
    return (
      ...
      {/*  <Text style={[globalStyles.priceText,{color:theme.color}]}>{item.price}</Text>*/}
         ...
         onPress={() => navigation.navigate('All People Works', item)}
         >
...
export default PeopleWorksCategory;
